...
import {DashboardData, SaveDashboardsRequest,} from '@/lib/interfaces/dashboardsData'
...
export async function getAllDashboards() {
  ...
  if (!res.ok) {
    ...
    console.log(`Failed to get dashboards: ${res.status} - ${errorText}`)
    throw new Error(
...
export async function saveDashboardData(data: DashboardData) {
  ...
  if (!res.ok) {
    const errorText = await res.text()
    console.log(`Failed to save dashboard: ${res.status} - ${errorText}`)
    throw new Error(
...
export async function setFavouriteDashboard(id: number) {
  ...
  if (!res.ok) {
    const errorText = await res.text()
    console.log(`Failed to set favourite dashboard: ${res.status} - ${errorText}`)
    throw new Error(
      `Failed to set favourite dashboard`
    )
  }
...
export async function deleteDashboardData(id: number) {
  ...
  if (!res.ok) {
    const errorText = await res.text()
    console.log(`Failed to delete dashboard: ${res.status} - ${errorText}`)
    throw new Error(
