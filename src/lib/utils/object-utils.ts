...
export const isGenericMimeType = (type?: string) =>
  !type || type === 'application/octet-stream' || type === 'binary/octet-stream'

export async function getFileUrlFromStream(
  response: Response,
  mimeTypeOverride?: string
): Promise<
  { url: string; fileName: string; detectedType: string } | undefined
> {
  ...
  // If a more specific MIME type is provided and the current one is generic, override it
  if (mimeTypeOverride && isGenericMimeType(blob.type)) {
    blob = new Blob([blob], { type: mimeTypeOverride })
  }
