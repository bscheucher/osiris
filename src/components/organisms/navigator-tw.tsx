      ...
      <div className="group relative flex items-center">
        ...
        <span className="ml-4 flex min-w-0 flex-col">
          ...
          {status !== STEP_STATUS.NEW && statusChangedBy && statusChangedOn && (
            <span className="text-sm text-gray-500" data-testid={`${title}-status-info`}>
              <span className="block 2xl:inline" data-testid={`${title}-changed-by`}>{statusChangedBy}</span>
              <span className="block 2xl:inline" data-testid={`${title}-changed-on`}>{statusChangedOn}</span>
            </span>
          )}
        </span>
      </div>
