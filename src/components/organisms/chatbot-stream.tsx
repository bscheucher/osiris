...
const UseCasePanel = ({
  ...
  <div
    className={twMerge(
      `w-full cursor-pointer overflow-hidden rounded-lg border bg-white hover:bg-gray-50`,
      className
    )}
...
const ChatMessage = ({ message }: { message: ChatMessage }) => (
  ...
  >
    <span
      className={twMerge(
        `text-normal inline-block max-w-[560px] rounded-lg px-4 py-3`,
        message.type === 'user'
...
const ChatbotStream: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    ...
    while (true) {
      i++ // count while cycles
      const { done, value } = await reader.read()

      // only set message every third step to save render cycles
      // and improve markdown rendering
      if (i % 4 === 0 || done) {
        setMessages((prev) => {
          const updatedMessages = [...prev]
          if (updatedMessages[updatedMessages.length - 1]?.type === 'bot') {
            updatedMessages[updatedMessages.length - 1].content = newMessage
          } else {
      ...
      if (text.includes('$thread_id')) {
        const threadId = text.split('|')[1]
        if (threadId.length) {
          setThreadId(threadId)
        }
      } else if (text.includes('$in_progress')) {
        // message creation was finished
        setIsRequesting(false)
      } else if (text.includes('$errorCode')) {
        // there was an error
        setIsRequesting(false)
        const error = text.split('|')[1]
        newMessage = `Entschuldigung, es gabe leider einen Fehler bei der Übertragung. Fehlercode: ${error}.`
      } else {
        // save all chunks to var
        newMessage += text
      }
    }
    ...
    <div className="chatbot-container fixed right-8 bottom-8 z-[100]">
      ...
      <div
        className={twMerge(
          ...
          isExpanded
            ? 'h-[calc(100vh-9rem)] w-[calc(100vw-22rem)]'
            : 'h-[800px] w-[560px]',
          isOpen ? 'chatbot-transform-open' : 'chatbot-transform-closed'
        )}
      >
        <div className="from-ibis-indigo to-ibis-blue-light flex flex-row items-center justify-between border-0 bg-gradient-to-br px-8 py-4">
          <button
            type="button"
          ...
          >
            ...
            {isExpanded ? (
              <ArrowsPointingInIcon
                aria-hidden="true"
                className="h-6 w-6"
                title={`Zu normaler Ansicht wechseln`}
              />
            ) : (
              <ArrowsPointingOutIcon
                aria-hidden="true"
                className="h-6 w-6"
                title={`Zu Vollbild-Ansicht wechseln`}
              />
            )}
          </button>
        ...
        <div className="form-container bg-white p-4">
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="relative flex min-h-13 flex-col-reverse rounded-2xl bg-gray-100">
              {/* <ButtonTw className="absolute bottom-3 left-3 bg-transparent p-2 text-gray-700 shadow-none hover:bg-transparent hover:text-gray-900">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <p
                    ...
                    data-placeholder="Fragen Sie AIVO..."
              ...
              <ButtonTw
                disabled={isRequesting}
                type="submit"
                className="absolute right-3 bottom-3 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-400 p-2 hover:from-emerald-600 hover:to-emerald-500 disabled:bg-gray-500 disabled:text-gray-200"
              >
